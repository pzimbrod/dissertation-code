...
class WeakForm(FEOperator):
    ...
    def __assemble_thermal_problem(self) -> None:
        ...
        self.residual_form += (
            ...
            + self.dt * (
            # Laplacian
            + kappa * self._laplacian(type="CG",test=test,u=T.next)
            # Right hand side
            - f * test * dx
            )
        )
    ...
    def __assemble_phase_problem(self) -> None:
        ...
        self.residual_form += (
            ...
            + self.dt * (
            # Advection
            - self._divergence(type="DG",test=test_s,u=u_solid,numerical_flux=self._upwind_vector)
            )
        )
        # Liquid fraction
        self.residual_form += (
            ...
            + self.dt * (
            # Advection
            - self._divergence(type="DG",test=test_l,u=u_liquid,numerical_flux=self._upwind_vector)
            )
        )
        # Gas fraction
        self.residual_form += (
            ...
            + self.dt * (
            # Advection
            - self._divergence(type="DG",test=test_g,u=u_gas,numerical_flux=self._upwind_vector)
            )
        )
    ...
    def __assemble_pressure_problem(self) -> None:
        ...
        self.residual_form += self._gradient(type="DG",test=test_u,u=p.next,numerical_flux=self._upwind_scalar)
    ...
    def __assemble_velocity_problem(self) -> None:
        ...
        self.residual_form += (
            ...
            + self.dt * (
                # continuity
                self._divergence("CG", test=test_p, u=u.next)
                # viscosity
                - self._laplacian(type="CG", test=test_u, u=u.next)
            )
        )